...
def ray_tracing_system(u, t, means_Gaussians, lam=1, sig: float = 0.1, A_: float = 0.1):
...
# Scipy Solver
def ray_tracing_solver(
    t, x0, y0, px0, py0, means_Gaussians, lam=1, sig: float = 0.1, A_: float = 0.1
):
    u0 = [x0, y0, px0, py0]
    # Call the ODE solver
    solPend = odeint(
...
def N_heads_run_Gaussiann_transfer(
    # energy_TL_weight=3,
    random_ic: bool = True,
    ...
    for i in range(1):
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=1000, gamma=scale
        )
        ...
        if not loadWeights:
            if random_ic:
                for j in range(number_of_heads):
                    ...
                    print(
                        "The initial condition (for y) is {}".format(initial_condition)
                    )
        ...
        with trange(num_epochs) as tepoch:
            for ne in tepoch:
                ...
                for l in range(number_of_heads):
                    ...
                    # Outputs
                    if parametrisation:
                        x = initial_x + (1 - torch.exp(-t)) * head[:, 0].reshape(
                            (-1, 1)
                        )
                        y = initial_y + (1 - torch.exp(-t)) * head[:, 1].reshape(
                            (-1, 1)
                        )
                    ...
                    for i in range(len(means)):
                        ...
                        potential = -A_ * torch.exp(
                            -(1 / (2 * sig**2)) * ((x - mu_x) ** 2 + (y - mu_y) ** 2)
                        )
                        # Partial wrt to x
        ...
        for i in range(number_of_heads):
            ...
            initial_y = Y0[i]
            x, y, px, py = ray_tracing_solver(
                t, x0, Y0[i], px0, py0, means_cell, sig=sig, A_=A_
            )
            if x[-1] > maximum_x:
                maximum_x = x[-1]
            if x[-1] < min_final:
            ...
            if parametrisation:
                ...
                px_ = 1 + (1 - torch.exp(-t_comparaison)) * trajectoires_xy[
                    :, 2
                ].reshape((-1, 1))
                py_ = 0 + (1 - torch.exp(-t_comparaison)) * trajectoires_xy[
                    :, 3
                ].reshape((-1, 1))
                ...
                ax[2].set_title(
                    "Difference between NN solution and numerical solution -x "
                )
            ...
            for m in range(len(means)):
                ...
                # Updating the energy
                H_curr_comparaison += -A_ * torch.exp(
                    -(1 / (2 * sig**2))
...
network_base = N_heads_run_Gaussiann_transfer(
    random_ic=False,
