...
class MyNetwork_Ray_Tracing(nn.Module):
    ...
    """
    def __init__(self, number_dims=40, number_dims_heads=10, depth_body=4,  N=1, Number_heads_TL=1):
...
def TL_N_heads_run_Gaussiann_transfer(network_in, specify_initial_condition=True, init_specified=0.55,\
                                   step_LR_step=1000, step_LR_gamma=0.9, adam_lr=0.01, sgd_momentum=0, \
                                   sgd_lr=0.005, energy_TL_weight=3, use_SGD_TL=False, \
                                   parametrisation=True, max_grid_grow=400, \
                                   TL_weighting=1, scale=0.7, A_=0.1, sig=.1,\
                                   initial_x=0, final_t=1, means=means_cell, \
                                   alpha_=1, width_=40, width_heads=10, \
                                   epochs_=25000, epochs_TL=25000, grid_size=400, \
                                   number_of_heads=11, number_of_heads_TL=11, \
  ...
  for i in range(1):
      ...
      if use_SGD_TL:
        optimizer50=optim.SGD(network50.parameters(), momentum=sgd_momentum, lr=sgd_lr)

      scheduler = torch.optim.lr_scheduler.StepLR(optimizer50, step_size=step_LR_step, gamma=step_LR_gamma)
      #device = torch.device("cuda")
      ...
      if not specify_initial_condition:
        for j in range(number_of_heads_TL):
          ...
          print('The initial condition (for y) for TL is {}'.format(initial_condition_TL))
      ...
      with trange(num_epochs_TL) as tepoch_TL:
        for ne in tepoch_TL:
            ...
            for l in range(number_of_heads_TL):
              ...
              # Outputs
              if parametrisation:
                x_TL=initial_x+(1-torch.exp(-t))*head_TL[:,0].reshape((-1,1))
                y_TL=initial_y_TL+(1-torch.exp(-t))*head_TL[:,1].reshape((-1,1))
              ...
              for i in range(len(means)):
                ...
                potential_TL=-A_*torch.exp(-(1/(2*sig**2))*((x_TL-mu_x)**2+(y_TL-mu_y)**2))
                # Partial wrt to x
                ...
                # Updating the energy
                H_0_TL+=-A_*math.exp(-(1/(2*sig**2))*((initial_x-mu_x)**2+(initial_y_TL-mu_y)**2))
                H_curr_TL+=-A_*torch.exp(-(1/(2*sig**2))*((x_TL-mu_x)**2+(y_TL-mu_y)**2))
              ...
              if not energy_conservation:
                # total loss
                loss_TL+=L1_TL+L2_TL+L3_TL+L4_TL+L5_TL+L6_TL+L7_TL+L8_TL
                # loss for current head
                lossl_val_TL=L1_TL+L2_TL+L3_TL+L4_TL+L5_TL+L6_TL+L7_TL+L8_TL
              if energy_conservation:
                # total loss
                loss_TL+=L1_TL+L2_TL+L3_TL+L4_TL+L5_TL+L6_TL+L7_TL+L8_TL+energy_TL_weight*L9_TL
                # loss for current head
                lossl_val_TL=L1_TL+L2_TL+L3_TL+L4_TL+L5_TL+L6_TL+L7_TL+energy_TL_weight*L9_TL
      ...
      try:
        print('The best loss (for TL) we achieved was:', temp_loss_TL, 'at epoch', epoch_mini_TL)
      ...
      torch.save({'model_state_dict': network2_TL.state_dict(), 'loss':temp_loss_TL,  
                  'epoch':epoch_mini_TL, 'optimizer_state_dict':optimizer50.state_dict(),
                  'total_epochs': total_epochs_TL, 'initial_condition': ic_TL},PATH)
      ###############################################################################
      ...
      'epochs_'+str(epochs_)+\
      'epochsTL_'+str(epochs_TL)+'grid_size_'+str(grid_size)+'Network_state'+'TL'+'.p'
      #os.mkdir(filename)
      f=open(filename,"wb")
      ...
      initial_y=ic_TL[0]
      # Saving the loss
      filename = 'Initial_x_'+str(initial_x)+'Initial_y_1_'+str(initial_y)+'final_t_'+str(final_t)+\
      'alpha_'+ str(alpha_)+'width_'+str(width_)+\
      'epochs_'+str(epochs_)+\
      'epochsTL_'+str(epochs_TL)+'grid_size_'+str(grid_size)+'loss'+'TL'+'.p'
      #os.mkdir(filename)
      f=open(filename,"wb")
      ...
      for m in range(number_of_heads_TL):
        ...
        filename = 'Head_'+str(m)+'Initial_x_'+str(initial_x)+'Initial_y_'+str(initial_y)+'final_t_'+\
        str(final_t)+'alpha_'+ str(alpha_)+'width_'+str(width_)+\
        'epochs_'+str(epochs_)+\
        'epochsTL_'+str(epochs_TL)+'grid_size_'+str(grid_size)+\
        'Trajectory_NN_x'+'TL'+'.p'
        #os.mkdir(filename)
        f=open(filename,"wb")
        ...
        str(final_t)+'alpha_'+ str(alpha_)+'width_'+str(width_)+\
        'epochs_'+str(epochs_)+\
        'epochsTL_'+str(epochs_TL)+'grid_size_'+str(grid_size)+'Trajectory_NN_py'+'TL'+'.p'
        #os.mkdir(filename)
        f=open(filename,"wb")
        ...
        if parametrisation:
          ...
          if print_legend:
            ax[1].plot(x_.cpu().detach(),y_.cpu().detach(), alpha=0.8, ls=':', label="NN solution (after TL) of SDE for {} head".format(m+1))
            t_p=np.linspace(-1,1,200)
            ax[0].loglog(range(num_epochs_TL),loss__TL, alpha=0.8, label='{} component of the loss'.format(m+1))
          else:
          ...
          if print_legend:
            ax[1].plot(uf_TL.cpu().detach()[:,0],uf_TL.cpu().detach()[:,1], label="NN solution (after TL) of SDE for {} head".format(m+1))
            t_p=np.linspace(-1,1,200)
            ax[0].loglog(range(num_epochs_TL),loss__TL, alpha=0.8, label='{} component of the loss'.format(m+1))
          else:
      ...
      for i in range(number_of_heads_TL):
          ...
          x, y, px, py = rayTracing_general(t, x0, Y0_TL[i], px0, py0, means_cell, sig=sig, A_=A_)
          if x[-1]>maximum_x:
            maximum_x=x[-1]
          if x[-1]<min_final:
            min_final=x[-1]
          ...
          'epochs_'+str(epochs_)+'grid_size_'+str(grid_size)+'Trajectories_x_TL'+'.p'
          #os.mkdir(filename)
          f=open(filename,"wb")
          pickle.dump(x,f)
          f.close()
          # Saving the (numerical trajectories)
          filename = 'Initial_x_'+str(initial_x)+'final_t_'+str(final_t)+'alpha_'+ str(alpha_)+'width_'+str(width_)+\
          'epochs_'+str(epochs_)+'grid_size_'+str(grid_size)+'Trajectories_y_TL'+'.p'
          #os.mkdir(filename)
          f=open(filename,"wb")
          pickle.dump(y,f)
          f.close()
          # Saving the (numerical trajectories)
          filename = 'Initial_x_'+str(initial_x)+'final_t_'+str(final_t)+'alpha_'+ str(alpha_)+'width_'+str(width_)+\
          'epochs_'+str(epochs_)+'grid_size_'+str(grid_size)+'Trajectories_px_TL'+'.p'
          #os.mkdir(filename)
          f=open(filename,"wb")
          pickle.dump(px,f)
          f.close()
          # Saving the (numerical trajectories)
          filename = 'Initial_x_'+str(initial_x)+'final_t_'+str(final_t)+'alpha_'+ str(alpha_)+'width_'+str(width_)+\
          'epochs_'+str(epochs_)+'grid_size_'+str(grid_size)+'Trajectories_py_TL'+'.p'
          #os.mkdir(filename)
          f=open(filename,"wb")
          pickle.dump(py,f)
          f.close()
          ...
          if parametrisation:
            ...
            x_=initial_x+(1-torch.exp(-t_comparaison))*trajectoires_xy_TL[:,0].reshape((-1,1))
            y_=initial_y+(1-torch.exp(-t_comparaison))*trajectoires_xy_TL[:,1].reshape((-1,1))
            px_=1+(1-torch.exp(-t_comparaison))*trajectoires_xy_TL[:,2].reshape((-1,1))
            py_=0+(1-torch.exp(-t_comparaison))*trajectoires_xy_TL[:,3].reshape((-1,1))

            # MSE: 
            MSE_TL=((x_.cpu().detach().reshape((-1,1))-x.reshape((-1,1)))**2).mean()+((y_.cpu().detach().reshape((-1,1))-y.reshape((-1,1)))**2).mean()
            MSE_TL+=((px_.cpu().detach().reshape((-1,1))-px.reshape((-1,1)))**2).mean()+((py_.cpu().detach().reshape((-1,1))-py.reshape((-1,1)))**2).mean()
            MSE_TL=MSE_TL/(4*Nt)
            ...
            # MSE: 
            MSE_TL=((trajectoires_xy_TL.cpu().detach()[:,0].reshape((-1,1))-x.reshape((-1,1)))**2).mean()+((trajectoires_xy_TL.cpu().detach()[:,1].reshape((-1,1))-y.reshape((-1,1)))**2).mean()
            MSE_TL+=((trajectoires_xy_TL.cpu().detach()[:,2].reshape((-1,1))-px.reshape((-1,1)))**2).mean()+((trajectoires_xy_TL.cpu().detach()[:,3].reshape((-1,1))-py.reshape((-1,1)))**2).mean()
            MSE_TL=MSE_TL/(4*Nt)
            # Should probably do a dict that saves them / save them to a file for the cluster
          ...
          ax[2].plot(t_comparaison.cpu().detach(),H0_init_TL[i]*np.ones(Nt), linestyle=':', c='r')
          ...
          for m in range(len(means)):
            ...
            # Updating the energy
            H_curr_comparaison_TL+=-A_*torch.exp(-(1/(2*sig**2))*((x_comparaison_TL-mu_x)**2+(y_comparaison_TL-mu_y)**2))
      ...
      ax[1].contourf(x1,y1,V, levels=20, cmap='Reds_r'); #ax[0].colorbar()
      ax[1].set_xlim(-.1,1.1)
      t1_TL=time.time()


      filename_fig = 'Initial_x_'+str(initial_x)+'Initial_y_'+str(initial_y)+'final_t_'+\
      str(final_t)+'alpha_'+ str(alpha_)+'width_'+str(width_)+\
      'epochs_'+str(epochs_)+'grid_size_'+str(grid_size)+'Trajectories_px_TL'+'.png'
...
for i in range(1):
  ...
  network_base=MyNetwork_Ray_Tracing(number_dims=40, number_dims_heads=10, N=11, Number_heads_TL=1)
  network_base.load_state_dict(torch.load(filename_saved, map_location=torch.device('cpu')))

  TL_N_heads_run_Gaussiann_transfer(network_base, specify_initial_condition=True, init_specified=[0.55],step_LR_step=1000, step_LR_gamma=0.995, sgd_lr=0.025, use_SGD_TL=True, parametrisation=True, max_grid_grow=400, \
                                    epochs_=25500, epochs_TL=25000, grid_size=400, \
